...
char picture_one_name[] = { "..\\..\\src\\2.tif" }; //在图片资源文件夹的路径后加入\\1.tif
...
Mat calibration::convertTo3Channels(const Mat& binImg)
{
...
void calibration::ProcessPicture(bool)
{
    ...
    timer->start(10); // 1000毫秒, 等于 1 秒
}

void calibration::Time_show()
{
    Mat  t_dst;
...
void calibration::Threshold_16(Mat &image)//将传入的16位图像 固定阈值二值化后 变成8位在返回
{
...
//计算灰度最大最小值
void calibration::MaxGrayValue16_8(Mat image, int &max, int &min)
{
    min = 65535;
    max = 0;
...
//自适应阈值 注：传入的只能是灰度图像
int calibration::adapthreshold16_8(Mat &image)
{
...
void calibration::dealcenter_signal(double *para)
{
...
void calibration::find_ball_one(Mat&image)
{
...
void calibration::compare_picture(Mat &image)
{
